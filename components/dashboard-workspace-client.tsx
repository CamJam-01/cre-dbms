...
type Props = { workspaceId?: string; currentUserId: string; members: WorkspaceMember[]; users: DirectoryUser[]; mode?: 'all' | 'create' | 'members' };

export function DashboardWorkspaceClient({ workspaceId, currentUserId, members: initialMembers, users, mode = 'all' }: Props) {
  ...
  const canManageMembers = members.some(member => member.user_id === currentUserId && member.role === 'admin');
  ...
  async function createWorkspace(event: FormEvent) {
    ...
    try {
      ...
      setCreateOpen(false); setWorkspaceName(''); router.push(`/dashboard?workspace=${result.workspace.id}`); router.refresh();
    } catch { setError('The Workspace could not be created. Please try again.'); }
  ...
  async function inviteMember(event: FormEvent) {
    event.preventDefault(); if (!workspaceId) return; resetMessages();
    ...
    setNotice(result.status === 'member' ? 'Existing user added to the Workspace.' : 'Invitation sent.'); setInviteUserId(''); setInviteEmail(''); await refreshMembers();
  }

  async function updateMember(userId: string, nextRole: string) {
    if (!workspaceId) return; resetMessages();
    ...
    setNotice('Member access updated.'); await refreshMembers();
  }

  async function removeMember(userId: string, label: string) {
    if (!workspaceId || !window.confirm(`Remove ${label}'s access to this Workspace?`)) return; resetMessages();
    const response = await fetch(`/api/workspaces/${workspaceId}/members`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId }) });
    const result = await response.json();
    if (!response.ok) { setError(result.error ?? 'The member access could not be removed.'); return; }
    setNotice('Member access removed.'); await refreshMembers();
  }
  ...
  return <>
    {workspaceId && mode !== 'create' && <section className="card dashboard-card">
      ...
      {canManageMembers && <form className="form-grid" onSubmit={inviteMember}>
      ...
      <div className="settings-list">{members.map(member => { const label = member.user?.full_name || member.user?.email || member.user_id; return <div className="settings-row" key={member.id}><span><strong>{label}</strong><small className="muted">{member.user?.email}</small></span>{member.user_id === currentUserId ? <span className="muted">Administrator · You</span> : canManageMembers ? <span className="actions"><select aria-label={`Role for ${member.user?.email ?? member.user_id}`} value={member.role} onChange={event => void updateMember(member.user_id, event.target.value)}><option value="viewer">Viewer</option><option value="editor">Editor</option><option value="admin">Administrator</option></select><button type="button" className="btn" onClick={() => void removeMember(member.user_id, label)}>Remove</button></span> : <span className="muted">{member.role === 'admin' ? 'Administrator' : member.role === 'editor' ? 'Editor' : 'Viewer'}</span>}</div>; })}</div>
    </section>}
    {mode !== 'members' && createOpen && <div className="dialog-backdrop" role="presentation" onMouseDown={() => !busy && setCreateOpen(false)}><div className="dialog-card" role="dialog" aria-modal="true" aria-labelledby="create-workspace-title" onMouseDown={event => event.stopPropagation()}><h2 id="create-workspace-title">Create Workspace</h2><p className="muted">You will be the only member initially. Invite other users after the Workspace is created.</p><form className="form-grid" onSubmit={createWorkspace}><div className="field"><label htmlFor="new-workspace-name">Workspace name</label><input id="new-workspace-name" autoFocus required value={workspaceName} onChange={event => setWorkspaceName(event.target.value)} placeholder="e.g. Acquisitions" /></div><div className="actions"><button type="button" className="btn" disabled={busy} onClick={() => setCreateOpen(false)}>Cancel</button><button className="btn primary" disabled={busy || !workspaceName.trim()}>{busy ? 'Creating…' : 'Create Workspace'}</button></div></form></div></div>}

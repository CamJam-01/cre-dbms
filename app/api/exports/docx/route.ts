...
export async function POST(request: Request) {
  try {
    ...
    const message = exportError instanceof Error ? exportError.message : 'The DOCX export could not be generated.';
    const userMessage = message.includes('Corrupted zip') || message.includes('End of data reached')
      ? 'The selected template is empty or corrupted. Upload a valid .docx template from Table Settings.'
      : message;
    return new NextResponse(`DOCX export failed: ${userMessage}`, { status: 500 });
  }
}

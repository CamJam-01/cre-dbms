...
export default function LandSalesPage() {
  const supabase = createClient(); const [rows, setRows] = useState<SaleRecord[]>([]); const [form, setForm] = useState(empty); const [editing, setEditing] = useState<string | null>(null); const [formOpen, setFormOpen] = useState(false); const [error, setError] = useState(''); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false);

...
export default function LandSalesPage() {
  ...
  function remove(id: string) { setPendingDelete(rows.find(row => row.id === id) ?? null); }
  ...
  return <main className="container"><div className="page-head"><div><h1>Land Sales</h1><div className="muted">Search, filter, sort, import, and export land transaction records.</div></div></div>{error && <div className="alert">{error}</div>}
    ...
    <ConfirmationDialog open={Boolean(pendingDelete)} title="Delete land sale?" description={pendingDelete ? `Delete ${pendingDelete.property_name} at ${pendingDelete.address}? This action cannot be undone.` : ''} busy={deleting} onConfirm={confirmRemove} onCancel={() => setPendingDelete(null)} />
  </main>;
}
